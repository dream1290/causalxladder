...
class ConfoundedLeverWorld:
    ...
    def step(self, action):
        ...
        # Interacting confounders: majority vote determines correct lever
        ...
        # Independent geometric flips for each bit
        self.flip_timers -= 1
...
class ContinuousAgent:
    def __init__(self, genome):
        ...
        self.belief = np.clip(genome[2], 0.1, 0.9)  # scalar average belief (proxy)
    ...
    def act(self, last_correct=False, last_act=None):
        ...
        return 1 if self.belief > 0.5 else 0
...
def mutate(genome):
    new_g = genome.copy()
    new_g += np.random.normal(0, 0.035, 6)
...
def evolve(name, hidden_dim, flip_mean, big_penalty, gens=200, pop=80):
...
results = {}
for d in dims:
    print(f"\n=== Scaling experiment: hidden_dim = {d} ===")
    ...
    for i, fm in enumerate(flip_means):
        for j, p in enumerate(penalties):
            ...
            eq_caps[i, j] = high[-1]  # final equilibrium
    results[d] = eq_caps

# Plot new heatmap for each d
fig, axs = plt.subplots(2, 2, figsize=(12, 10))
axs = axs.ravel()
for idx, d in enumerate(dims):
    im = axs[idx].imshow(results[d], cmap='RdYlBu_r', origin='lower', vmin=0.0, vmax=0.65)
    axs[idx].set_title(f'hidden_dim = {d}')
...
plt.suptitle('Scaling Hidden State Dimensionality — Does the Phase Transition Survive?')
plt.tight_layout()
plt.savefig('scaling_heatmap.png')
plt.show()

print("Scaling experiment complete. Check scaling_heatmap.png")