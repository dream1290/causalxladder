"""
CLW-3: Evaluation Suite
=======================
Runs any BenchmarkAgent through the CLW-3 common cause tests.

The key CLW-3 test: do(S1) does NOT change C or the optimal action.
An agent that fails to distinguish symptom from cause will react incorrectly.
"""
...
from .interventions import CLW3InterventionProtocol, EpisodeSpec, InterventionSpec
...
def evaluate_agent(
    ...
    protocol: Optional[CLW3InterventionProtocol] = None,
) -> ScoringMatrix:
    if protocol is None:
    ...
    if _agent_has_representation(agent):
        ...
        if b_similarities:
            ...
            matrix.set_score(ENV_NAME, 'B-full', TestScore(
                mean_sim, classify_level_b(mean_sim),
                max(0.0, 1.0 - float(np.std(b_similarities)))
            ))
    ...
    for novel_type, tag in [('C1_do_cause', 'C1'), ('C2_noop', 'C2')]:
...
def _run_episode_test_a(agent: BenchmarkAgent, ep: EpisodeSpec) -> list:
    env = CLW3Environment(seed=ep.seed, max_steps=200)
    obs = env.reset()
    agent.reset()
    agent.observe(obs)
...
def _apply_intervention(env: CLW3Environment, intv: InterventionSpec) -> None:
...
def _expected_representation(gt: dict) -> np.ndarray:
    """Expected representation: [C, S1, S2, 1.0]."""
    return np.array([gt['C'], gt['S1'], gt['S2'], 1.0], dtype=np.float32)
